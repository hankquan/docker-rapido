...
public class ServiceTaskHandler {
	...
	public void runTask() {
		...
		if (isBuildImage) {
			...
			logger.info("Find local image with same image-tag: {}, rapido will try to remove it after deployment", imageName);
		...
		if (existedImageId != null) {
			try {
				optDocker.removeImage(existedImageId);
			} catch (Exception e) {
				logger.warn("Removing local image skipped for it is using by some other containers");
			}
		}
