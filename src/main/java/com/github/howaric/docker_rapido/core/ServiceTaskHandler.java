...
public class ServiceTaskHandler {
	...
	public ServiceTaskHandler(RapidoTemplate rapidoTemplate, String serviceName, List<Node> targetNodes,
			String imageTag) {
		super();
	...
	public void runTask() {
		...
		logger.info("Get service task: {}, to build image-tag: {}, rapido will deploy this service to nodes: {}",
				serviceName, imageTag, targetNodes);
		// build image if need
		...
		if (service.getBuild() != null) {
			if (imageTag == null) {
				if (DeliveryType.isDevelopmental(rapidoTemplate.getDeliver_type())) {
					imageTag = rapidoTemplate.getOwner();
					isBuildImage = true;
				} else {
					throw new IllegalImageTagsException(
							"Image tag can not be empty when build is specific in official deployment");
				}
			} else {
				isBuildImage = true;
			}
		...
		if (isBuildImage) {
			...
			optDocker.pushImage(imageName, rapidoTemplate.getRepository().getUsername(),
		...
		String deploy_policy = service.getDeploy().getDeploy_policy();
		...
		for (Node node : targetNodes) {
			...
			deployer.deploy(rapidoTemplate.getRepository(), rapidoTemplate.getDeliver_type(), rapidoTemplate.getOwner(),
					node, serviceName, service, imageName);
		}
