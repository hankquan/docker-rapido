...
import com.github.howaric.docker_rapido.excutors.ExecutorsCentre;
...
public class RapidoEngine {
	...
	public void startRapido() {
		...
		/**
		 * validate template dependence and etc; check node connectivity sort out
		 * template
		 */
		List<String> orderedServices = validateDependence(rapidoTemplate);
		...
		for (int i = 0; i < orderedServices.size(); i++) {
			...
			if (service.getDeploy() != null) {// deploy nodes
				targetNodes = service.getDeploy().getPlacement().getNodes(nodes);// consider multi-thread
			}
			...
			if ((targetNodes == null || targetNodes.isEmpty()) && imageTag == null) {
				throw new GenerateRapidoJobFailedException(
						"TargetNodes and imageTag are empty at the same time, nothing has to do");
			}
			logger.info(serviceName + "==targetNodes:" + targetNodes);
			JobModel jobModel = new JobModel(rapidoTemplate.getDeliver_type(), rapidoTemplate.getOwner(), serviceName,
					service, targetNodes, imageTag);
	...
	private String getImageTag(String serviceName) {
		for (String tags : imageTags) {
			if (tags.contains(":")) {
				...
				throw new IllegalImageTagsException(
						"Image tags should be serviceName:tag when there are more than one");
			} else {
				return tags;
			}
		}
	...
	private List<String> validateDependence(RapidoTemplate rapidoTemplate) {
